...
- **Sticky nav** con sombra al scroll · 14 secciones ancladas (`#inicio`, `#resumen`, `#capi`, `#capii`, `#capiii`, `#capiv`, `#datos`, `#mlops`, `#tablas`, `#reproducibilidad`, `#glosario`, `#referencias`, `#autores`, `#contacto`).
...
| **Base de datos** (`#datos`) | §3.1.5–§3.1.6 + Cap IV Fase 1 + repo `VisaPredictAI` | **NUEVA SECCIÓN 14-jun-2026.** Documenta la construcción del panel $y_{p,c,b,t}$ (Objetivo 1): banda de métricas (194 series · 27,127 obs · 290 boletines dic-2001→2026), 3 diagramas SVG (pipeline 6 etapas, odisea EB-5, distribución de estado F/C/U), 5 brechas H1–H5 + bonus robustez, búsqueda profunda 290/295 (5 muertos Wayback-only), mega-auditoría 12 dim (APTO). Cifras = artefacto de datos, NO resultados predictivos. Coherente con `.tex` reconciliado (dic-2001 ~290 obs, t₀=1975, [14]) |
| **MLOps** (`#mlops`) | Ingeniería del repo `VisaPredictAI` | **NUEVA SECCIÓN 14-jun-2026.** Documenta las prácticas MLOps: banda de métricas (27 tests · 2 workflows CI · ~70% cobertura · ruff+mypy · 0 críticos), diagrama SVG del **doble pipeline** (CI de código push/PR + refresco de datos diario con gate + issue-en-fallo), 6 tarjetas de robustez (retry, sort determinista, abort masivo, alertas, centinela UNK, hallado-por-tests), contrato/linaje + versionado (CSV en git, figuras no, DVC futuro). Sincronizado con `.tex` (FAD 1992 + caveat dic-2001 ~290 obs). Describe ingeniería, NO resultados predictivos |
| **Estructura** (`#estructura`) | Estructura del repo `VisaPredictAI` (cookiecutter-data-science) | **NUEVA SECCIÓN 15-jun-2026.** Documenta la plantilla **Cookiecutter Data Science (CCDS)** aplicada al repo: árbol comentado (`<pre>` mono), `data/raw` vs `data/processed`, `reports/`+`docs/`, 6 cards de mapeo (raw/processed/reports/tests/núcleo/pyproject), bloque de comandos `make`, y 4 adaptaciones honestas (MIT, paquete instalable, sin `notebooks/`·`models/` aún, sin `.env`). Describe organización del repo, NO resultados predictivos |
