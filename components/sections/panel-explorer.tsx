  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap gap-4 rounded-xl border border-border bg-card p-4">
        <Select label="País / área de cargabilidad" value={country} onChange={setCountry} options={panel.countries} fmt={countryLabel} />
        <Select label="Categoría" value={category} onChange={setCategory} options={cats} />
        <Select label="Tabla" value={table} onChange={setTable} options={panel.tables} />
      </div>

      <ChartCard
        title="Evolución de la fecha de prioridad"
        desc={`${countryLabel(country)} · ${category} · ${table}. Días desde la fecha base (t₀ = 1975); las caídas son retrogresiones reales.`}
      >
        {hasSeries ? (
          ...
          <NoData msg="Esta combinación no publicó fechas específicas (sólo C/U): sin serie numérica." />
        )}
      </ChartCard>

      <div className="grid gap-4 lg:grid-cols-2">
        <ChartCard title="Comparación entre países" desc={`${category} · ${table}. Disparidad de rezago por el límite del 7 % anual.`}>
          {hasComparison ? (
            ...
            <NoData msg="Sin datos para esta categoría/tabla." />
          )}
        </ChartCard>

        <ChartCard title="Movimiento mes a mes" desc={`${countryLabel(country)} · ${category} · ${table}. Verde avanza, rojo retrocede.`}>
          {hasMovement ? (
            <ResponsiveContainer width="100%" height={300}>
            ...
            <NoData msg="Sin movimientos calculables para esta serie." />
          )}
        </ChartCard>
      </div>

      <ChartCard
        title="Distribución de estado administrativo C / F / U"
        desc="Calculado sobre las observaciones reales del panel. F (fecha específica) es el único objetivo predictivo."
      >
      ...
      <div>
        <h3 className="mb-1 font-serif text-xl font-bold">Tabla histórica completa</h3>
        <p className="mb-3 text-sm text-muted-foreground">
          Las {panel.rows.length.toLocaleString("es-MX")} observaciones del panel,
          virtualizadas. Ordene por columna, filtre, ajuste columnas y exporte la
          vista filtrada a CSV.
        </p>
        <div className="mb-3 flex flex-wrap gap-3 rounded-xl border border-border bg-card p-4">
          <Select label="País" value={fCountry} onChange={setFCountry} options={["todos", ...panel.countries]} fmt={(v) => (v === "todos" ? "Todos" : countryLabel(v))} />
          <Select label="Categoría" value={fCategory} onChange={setFCategory} options={["todos", ...cats]} fmt={(v) => (v === "todos" ? "Todas" : v)} />
          <Select label="Tabla" value={fTable} onChange={setFTable} options={["todos", ...panel.tables]} fmt={(v) => (v === "todos" ? "Todas" : v)} />
          <Select label="Estado" value={fStatus} onChange={setFStatus} options={["todos", ...statuses]} fmt={(v) => (v === "todos" ? "Todos" : v)} />
        </div>
        {tableRows.length ? (
          <PanelTable rows={tableRows} />
        ) : (
          <NoData msg="Sin filas para esta combinación de filtros." />
        )}
      </div>
    </div>
  );
