...
const BASE_MAP = {
  ...
  // F1: el catálogo de modelos vive aquí. El artefacto ya viajaba en el release como
  // `critical`; lo único que faltaba era consumirlo, y por eso el conteo estaba tecleado.
  "reports/governance/key_facts.json": "key_facts.json",
  ...
  // E5: las cifras de la épica de cohortes y el catálogo del que sale la cohorte de cada
  // serie. Viajan como `required`, así que un corte puede no traerlos: el consumidor NO
  // inventa nada cuando faltan, simplemente no muestra cohortes ni cifras de cohorte.
  "reports/governance/e5_facts.json": "e5_facts.json",
  "reports/eval/series_cohorts.json": "series_cohorts.json",
};
...
export function validateArtifact(contract, buffer) {
  ...
  // R0-03 (reauditoría ciega): rutas anidadas con tipo — espejo de check_contracts.py.
  for (const [dotted, t] of Object.entries(contract.required_paths ?? {})) {
    let node = data;
    for (const part of dotted.split(".")) {
      node = node && typeof node === "object" && !Array.isArray(node) ? node[part] : undefined;
      if (node === undefined) return `ruta requerida ausente: ${dotted}`;
    }
    if (!JS_TYPE[t](node)) return `'${dotted}' debería ser ${t}`;
  }
  return true;
}

// F1 · El tamaño del catálogo de modelos, leído del artefacto gobernado y NUNCA tecleado.
// Falla antes de emitir nada: sin fallback y con el motivo exacto, porque un número de
// portada equivocado es peor que un build roto. El contrato de key_facts no declara
// `n_models` (comprobado), así que su forma se exige aquí además de validar el artefacto.
export function modelCount(contract, buffer) {
  const valid = validateArtifact(contract, buffer);
  if (valid !== true) throw new Error(`key_facts.json: contrato violado (${valid})`);
  const { n_models: n } = JSON.parse(buffer.toString("utf8"));
  if (n === undefined) throw new Error("key_facts.json: falta n_models — el catálogo de modelos no se puede derivar");
  if (!Number.isInteger(n)) throw new Error(`key_facts.json: n_models debería ser un entero, llegó ${JSON.stringify(n)}`);
  if (n <= 0) throw new Error(`key_facts.json: n_models debe ser positivo, llegó ${n}`);
  return n;
}
 ...
 * @returns {Promise<{ swapped: number, rolledBack: boolean, unrecovered?: string[], error?: string }>}
 */
export async function executeSwap(entries, results, { out, staging, backup, joinPath, dirOf, fs }) {
  ...
  try {
    ...
    // Rollback FAIL-CLOSED (2ª ronda de auditoría 12-jul: los errores de restauración se
    // tragaban con .catch(()=>{}) y luego se BORRABA el backup — un fallo doble perdía
    // archivos y aún reportaba rolledBack:true). Orden: restaurar cada backup ENCIMA de
    // lo que haya en dest (rename sobrescribe — cubre el caso placed+backed), luego
    // retirar los nuevos SIN versión previa. Cualquier fallo aquí deja el backup EN
    // DISCO y regresa rolledBack:false + unrecovered — el caller debe reventar el build.
    const unrecovered = [];
    for (const o of movedAside) {
      try {
        await fs.mkdir(dirOf(joinPath(out, o)));
        await fs.rename(joinPath(backup, o), joinPath(out, o));
      } catch {
        unrecovered.push(o);
      }
    }
    for (const o of placed) {
      if (movedAside.includes(o)) continue; // el restore de arriba ya lo sobrescribió
      try {
        await fs.rm(joinPath(out, o));
      } catch {
        unrecovered.push(o);
      }
    }
    ...
    if (unrecovered.length) {
      // NO borrar el backup: es la única copia de lo no restaurado.
      return { swapped: 0, rolledBack: false, unrecovered, error };
    }
    await fs.rm(backup).catch(() => {}); // ya vacío de archivos: todos se restauraron
    return { swapped: 0, rolledBack: true, error };
  }
}

// Ronda 3 audit: la decisión sobre el resultado del swap es LÓGICA PURA y vive aquí,
// testeada — el script solo ejecuta la disposición (abort ⇒ process.exit(1): un build
// muerto nunca deploya). unrecovered manda sobre todo lo demás.
/**
 * @param {{ swapped: number, rolledBack: boolean, unrecovered?: string[], error?: string }} swap
 * @returns {{ kind: "fresh" } | { kind: "stale", reason: string } | { kind: "abort", message: string }}
 */
export function swapDisposition(swap) {
  if (swap.unrecovered?.length) {
    return {
      kind: "abort",
      message: `rollback INCOMPLETO (${swap.error}) — sin restaurar: ${swap.unrecovered.join(", ")}`,
    };
  }
  if (swap.rolledBack) return { kind: "stale", reason: `swap rollback: ${swap.error}` };
  return { kind: "fresh" };
}

// Ronda 4: el EFECTO del abort también corre bajo test — error() y exit() se inyectan
// (el script pasa console.error/process.exit). Tras esto, lo único fuera de test es
// el binding de esos dos argumentos en fetch-data.
/**
 * @param {{ kind: string, message?: string, reason?: string }} disposition
 * @param {{ backup: string, error: (m: string) => void, exit: (code: number) => void }} io
 */
export function executeDisposition(disposition, { backup, error, exit }) {
  if (disposition.kind !== "abort") return;
  error(`fetch-data: ${disposition.message}; backup preservado en ${backup} — FALLANDO el build`);
  exit(1);
}
