...
export function parseForecastStore(csv, metaJson, scorecardJson) {
  ...
  if (meta.horizon_months != null && !(Number.isFinite(meta.horizon_months) && meta.horizon_months >= 0))
    return unavailableStore("malformed_forecast_meta"); // Infinity/NaN horizon → closed
  ...
  // >= half corrupt (incl. EXACTLY 50%) is a broken feed, not a servable one.
