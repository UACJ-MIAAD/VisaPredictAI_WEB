...
export function parseForecastStore(csv, metaJson, scorecardJson) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // reject a corrupt point: non-finite number, bad date, or inverted/incoherent bands
    if (![days, lo80, hi80, lo95, hi95].every(Number.isFinite) || !DATE_RX.test(a[I.d] ?? "") ||
        lo80 > hi80 || lo95 > hi95 || lo95 > lo80 || hi95 < hi80) { droppedRows++; continue; }
